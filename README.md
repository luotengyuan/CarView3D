...
CarView3D 是一个原生 Android 3D 车模展示示例项目，提供三种交互模式：序列图片 360 展示、视频 360 展示和 GLB 实时模型展示。用户可通过左右滑动控制车模旋转，并支持自动播放、触摸中断、延迟恢复等交互行为。

## GitHub 首页展示版简介

一个基于 Android 原生开发的 3D 车模展示 Demo，支持图片序列、视频和 GLB 模型三种旋转方案，具备左右滑动跟手控制、方向记忆和自动播放恢复能力。
...
- 增加首页模式入口（图片模式 / 视频模式 / 模型模式）
- 新增视频 3D 展示页面与滑动联动交互
- 新增 GLB 模型 3D 展示页面（Sceneform + Filament）
...
- 首页模式选择：
  ...
  - `GlbCarActivity`：基于 `res/raw/stylized_car_unlit_noground.glb` 的实时 3D 模型展示
- 手势交互：
  - 向左滑：向后旋转
  - 向右滑：向前旋转
- 自动播放策略：
...
- 主要依赖：
  ...
  - Sceneform（维护版）`com.gorisse.thomas.sceneform:sceneform:1.23.0`
...
│   │   └── activity_glb_car.xml
│   └── src/main/res/raw/
│       ├── car_view.mp4
│       ├── car_view2.mp4
│       ├── stylized_car.glb
│       ├── stylized_car_fixed.glb
│       ├── stylized_car_unlit_fixed.glb
│       └── stylized_car_unlit_noground.glb
├── gradle/
├── build.gradle
├── settings.gradle
└── gradlew / gradlew.bat
...
- 模拟器图形后端不同，视频/GLB 渲染表现可能有差异，建议同时在真机验证。
